...
const Experiment = require(path.join(__dirname, '../server/models/Experiment'));
...
const seedGovernmentDemo = async () => {
  try {
    ...
    await User.deleteMany({ email: { $regex: /demo|admin|test/ } });
    await Experiment.deleteMany({ category: { $in: ['water-testing', 'waste-recycling'] } });
    ...
    waterQualityExperiment.submissions.push({
      user: students[1]._id,
      observations:
        'Water from school well. pH = 6.8, slightly cloudy, mild earthy odor. Found a mosquito larva.',
      results: 'Well water is slightly acidic. Shows signs of potential contamination from stagnant water.',
      learnings: 'Found out that well water needs more testing than tap water.',
      photos: [],
      rating: 4,
      status: 'pending',
    ...
    waterQualityExperiment.submissions.push({
      user: students[2]._id,
      observations: 'Rainwater collected from terrace. pH = 6.5, clear, no odor.',
      results: 'Rainwater is slightly acidic but appears clean. Could be used for plants.',
      learnings: 'Rainwater is generally clean and can be harvested for non-drinking purposes.',
      photos: [],
      rating: 5,
      status: 'approved',
    ...
    waterQualityExperiment.submissions.push({
      user: students[3]._id,
      observations: 'Did not specify water source. pH value unclear.',
      results: 'No clear results provided.',
      learnings: 'Not specified.',
      photos: [],
      rating: 2,
      status: 'rejected',
      submittedAt: new Date(Date.now() - 6 * 24 * 60 * 60 * 1000),
      reviewedAt: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000),
      teacherFeedback: 'Please specify the water source and provide clear pH reading. Try again.',
      points: 0
    });
    ...
    // Submission 1: Approved
    wasteSegregationExperiment.submissions.push({
      user: students[0]._id,
      observations:
        'Collected waste from home for 1 day. Segregated into 6 categories. Organic waste was most abundant.',
      results:
        'Total items: Organic=15, Paper=8, Plastic=12, Metal=2, Glass=3, Others=1. Total=41 items.',
      learnings:
        'Learned that most waste in my home is organic. Proper segregation can help in composting and recycling.',
      photos: [],
      rating: 5,
      status: 'approved',
    ...
    wasteSegregationExperiment.submissions.push({
      user: students[1]._id,
      observations:
        'School waste audit. Collected from 3 classrooms for half day. Large amount of paper waste from notebooks.',
      results:
        'Organic=5, Paper=28, Plastic=15, Metal=1, Glass=0, Others=8. Most waste from school is paper.',
      learnings: 'Schools generate lot of paper waste. Should implement digital systems to reduce it.',
      photos: [],
      rating: 4,
      status: 'pending',
    ...
    wasteSegregationExperiment.submissions.push({
      user: students[2]._id,
      observations: 'Community waste audit from market area. Mixed waste types. Found many plastic bags.',
      results:
        'Organic=20, Paper=5, Plastic=35, Metal=3, Glass=5, Others=12. Plastic is major waste type.',
      learnings: 'Markets generate huge plastic waste. Vendors should use paper bags instead of plastic.',
      photos: [],
      rating: 4,
      status: 'approved',
      submittedAt: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000),
